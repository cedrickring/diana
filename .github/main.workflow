...
action "Upload darwin release" {
  ...
  args = "out/diana_darwin_amd64 application/octet-stream"
...
action "Upload linux release" {
  uses = "JasonEtco/upload-to-release@master"
  args = "out/diana_linux_amd64 application/octet-stream"
  secrets = ["GITHUB_TOKEN"]
  needs = ["Build all binaries"]
}
...
action "Upload windows release" {
  uses = "JasonEtco/upload-to-release@master"
  args = "out/diana_windows_amd64.exe application/octet-stream"
  secrets = ["GITHUB_TOKEN"]
  needs = ["Build all binaries"]
}
